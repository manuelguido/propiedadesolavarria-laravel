...
class ModeratorUpdateTest extends TestCase
{
    private $apiUtils;
    private $authHeaders;
    ...
    protected function setUp(): void
    {
        ...
        $authorizedToken = $this->apiUtils
            ->getAnyUserByPermission(Permission::ModeratorUpdate)
        ...
        $unauthorizedToken = $this->apiUtils
            ->getAnyUserWithoutPermission(Permission::ModeratorUpdate)
            ->createToken('TestToken')->plainTextToken;
        ...
        // Route
        $this->route = '/api/v1/moderator/update/' . Moderator::inRandomOrder()->first()->moderator_id;
        ;
    }

    public function test_moderator_update_returns_json()
    {
        $response = $this->patchJson($this->route, $this->newData, $this->authHeaders);
        $this->assertJson($response->getContent());
    }

    public function test_moderator_update_returns_code_201()
    {
        $response = $this->patchJson($this->route, $this->newData, $this->authHeaders);
        $response->assertStatus(201);
    }

    public function test_moderator_update_returns_correct_information()
    {
        $response = $this->patchJson($this->route, $this->newData, $this->authHeaders);
        $this->assertEquals($this->newData['name'], $response->json()['data']['name']);
    }

    public function test_moderator_update_returns_json_with_unathorized_user()
    {
        $response = $this->patchJson($this->route, $this->newData, $this->unauthHeaders);
        $this->assertJson($response->getContent());
    }

    public function test_moderator_update_returns_code_403_with_unathorized_user()
    {
        $response = $this->patchJson($this->route, $this->newData, $this->unauthHeaders);
