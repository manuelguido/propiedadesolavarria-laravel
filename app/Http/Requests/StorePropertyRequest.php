    ...
    public function rules()
    {
        return [
            ...
            'images.*.order' => [
                ...
                'integer',
                'min:1',
                Rule::unique('property_image', 'order')
                    ->where(function ($query) {
                        return $query->where('property_id', $this->property_id);
                    })
            ],
            // 'images.*.file' => 'required|image|mimes:pdfjpeg,png|max:2048',
//             'images.*.order' => 'required|integer|min:1|unique:property_image,order,NULL,id,property_id,' . $this->property_id,
        ];
    }
